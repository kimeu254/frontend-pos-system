...
import {API_CATEGORIES_URL, DEFAULT_UPLOAD_IMG} from "@/image";
...
const getCategory = async () => {
    try
    {
        ...
        if (res.data.category.image)
        {
            image.value = API_CATEGORIES_URL+res.data.category.image
        }
        else
        {
...
const onSubmit = async () => {
    ...
    try
    {
        ...
        let res = await axios.post('api/category/edit/' + id + '?_method=PUT', data)
...
const deletePicture = async (id) => {
    try
    {
        await Swal.fire({
            ...
            {
                axios.delete('api/category/image/remove/' + id).then((res) => {
                    Swal.fire({
                        title: 'Deleted!',
...
<template>
    <h5>Edit Category</h5>
    <div class="row my-5">
        <CardComponent col-class="col-lg-6" label="Update Category">
            <template #content>
